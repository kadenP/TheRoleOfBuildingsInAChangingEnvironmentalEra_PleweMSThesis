...
'''dictionary containing the most sensative parameters based on DGSM index'''
paramSetOpt = {}
paramSetOpt['input'] = []
...
'''List of top parameters in terms of DGSM sensativity for Facility Electric Energy'''
highDGSM = [853, 839, 880, 865, 870]
# highDGSM = [853, 839, 880, 865, 870, 860, 875, 653, 188, 730]
    ...
    for object in idfdevice.idfobjects[objectClass]:
        ...
        for fieldname in object.fieldnames:
            ...
            if any( [type(object[fieldname]).__name__ == 'int', type(object[fieldname]).__name__ == 'float']) and \
                ...
                if len(paramSet['input']) - 1 in highDGSM:
                    paramSetOpt['input'].append({'ID': len(paramSet['input']) - 1,
                                              'Object Name': objectName,
                                              'Field Name': fieldname,
                                              'Nominal Value': object[fieldname],
                                              'Scaled Nominal Value': nomValNorm,
...
filename = 'jsonOUTPUT_ALL_Train.txt'
with open(filename, 'w') as outfile:
    json.dump(paramSet, outfile)
print('%.0f numerical parameters were found and downloaded to %s' % (len(paramSet['input']), filename))

filename = 'jsonOUTPUT_FEEOpt5.txt'
with open(filename, 'w') as outfile:
    json.dump(paramSetOpt, outfile)
print('%.0f numerical parameters were found and downloaded to %s' % (len(paramSetOpt['input']), filename))
...
--------------------------------------------------------------------------------------------------------'''
'''JSON parameters'''
with open('jsonOUTPUT_FEEOpt10.txt') as jsonParams:
